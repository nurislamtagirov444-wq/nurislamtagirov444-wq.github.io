...
Версия спеки: **1.2** · Дата: 2026-08-02
...
- 1.2: арт-дирекшн изменён по решению заказчика: **классические японские визуальные
  новеллы ~2000-х годов** (аниме сел-шейдинг, длинные волосы у героинь, взрослые
  пропорции — героини явно читаются как 27/26-летние). Оригинальные дизайны персонажей;
  запрет на копирование конкретных героинь существующих игр остаётся (§0). Реалистичный
  живописный стиль v1.0–1.1 полностью заменяется.
...
## Приложение A. Статус реализации (после срезов P0, 2026-08-02)

P0 выполнен по E1–E13. Арт-дирекшн v1.2 (классические японские VN) применена: 6 портретов
(Марина ×3 — длинные волосы, Дарья ×2, Штерн) + 7 фонов; 3 фона из «запасной волны»
лимита генерации приведены к стилю локальной тунификацией (scripts/toonify.py).
Отдельные CG концовок не сгенерированы (лимит 10/сессия): экраны концовок работают на
стилизованных композициях фонов с цветокором — догенерация CG в P1. Проверки: 11/11
unit, автоплей (все 4 концовки достижимы), E2E 24/24 × 2 прогона подряд в headless
Chromium, `tsc --noEmit` чист, `vite build` зелёный, консоль рантайма без ошибок.
